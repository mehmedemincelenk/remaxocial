...
export default function Step3_MusteriVideosu({ sessionId, memberId, onComplete, onPrev }) {
  ...
  const handleSave = async () => {
    if (!file) return onComplete('musteri_video_skipped');
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage.from('test-musteri-video').upload(filePath, file);
    ...
    } catch (e) { notify('Yükleme hatası.', 'error'); } finally { setIsSubmitting(false); }
  };
  ...
  return (
    <GlassCard padding="1.1rem" borderRadius="18px" style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', position: 'relative' }}>
      ...
      <div style={{ textAlign: 'center' }}><h3 style={{ margin: 0, fontSize: '1.15rem' }}>🎥 Müşteri Videosu</h3><p style={{ color: '#aaa', fontSize: '0.78rem', margin: '4px 0 0 0' }}>Varsa yükleyin (Opsiyonel).</p></div>
      <div style={uploadBoxStyle}>
        <input type="file" accept="video/*" onChange={e => setFile(e.target.files[0])} style={fileInputStyle} />
        <UploadCloud size={28} color={file ? 'var(--color-accent)' : '#444'} />
        <span style={{ color: file ? '#fff' : '#888', fontSize: '0.8rem' }}>{file ? file.name : 'Video Seçin'}</span>
      </div>
      <div style={{ display: 'flex', gap: '0.8rem' }}>
        <button onClick={() => onComplete('musteri_video_skipped')} style={secondaryBtnStyle}>Atla</button>
        <button onClick={handleSave} disabled={isSubmitting} style={primaryBtnStyle}>{isSubmitting ? '...' : 'İlerle'}</button>
      </div>
    </GlassCard>
  );
}
